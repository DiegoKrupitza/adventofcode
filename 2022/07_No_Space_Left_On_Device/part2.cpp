...
struct TreeNode {
    ...
    TreeNode* makeSpaceFor(int spaceNeeded)
    {
        ...
        for(auto& child : this->children)
        {
            ...
            if(child.size >= spaceNeeded) {
                auto subresult = child.makeSpaceFor(spaceNeeded);
...
int main() {
    ...
    auto nodeToDelete = root.makeSpaceFor(30000000 - (70000000-(root.size)));
    std::cout << "Node to delete " << nodeToDelete->name << " size: " << nodeToDelete->size << std::endl;
