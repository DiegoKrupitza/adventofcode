...
public:
    ...
    Monkey(std::list <ui64> elements, std::function<ui64(ui64)> updateFunction,
           std::function<size_t(ui64)> forwardFunction,
           ui64 divisor) :                                elements(elements),
                                                          updateFunction(updateFunction),
                                                          forwardFunction(forwardFunction),
                                                          divisor(divisor) {
    }

    std::vector <std::pair<size_t, ui64>> process(ui64 div) {
...
int main() {
    ...
    // the trick is to see that you can modulo by the multi of all divisors 
    // didnt see the trick and only found that thanks to 
    // https://github.com/tbeu/AdventOfCode/blob/master/2022/day11/day11.cpp
    const auto div = std::accumulate(monkeys.begin(), monkeys.end(), ui64{1},
                                         [](ui64& p, const Monkey& monkey) { return p * monkey.divisor; });

    for (int round = 1; round <= 10000; round++) {
        LOG("Start Round: " << round);

        for (auto &monkey: monkeys) {
            const auto forwards = monkey.process(div);
    ...
    for (auto &monkey: monkeys) {
        ...
        LOG("Inspects of Monkey: " << inspects);
